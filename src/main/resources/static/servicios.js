...
if (favBtn) {
  favBtn.addEventListener("click", () => {
    ...
    if (existe) {
      ...
      document.getElementById("cant_fav").innerText = listaFavoritos.children.length;
...
document.getElementById("generar-reco").addEventListener("click", () => {
  ...
  if (arrayFavoritos.length == 0) {
    Swal.fire({
      title: "Error",
      text: "Selecciona por lo menos un anime",
      icon: "error",
      theme: "dark"
    });
    return
  ...
  fetch("/generarRecomendaciones", {
    ...
    .then(data => {
      ...
      recomendaciones.forEach(anime => {
        ...
        card.className = "col";
        card.innerHTML = `
          <article class="card h-100 shadow-sm mx-auto text-bg-dark border border-secondary rounded">
        ...
        Swal.fire({
          title: "Enhorabuena",
          text: "Recomendaciones creadas correctamente",
          icon: "success",
          theme: "dark"
        });
      });
